...
class SearchEngine
    ...
    def get_instanse data
        # create a instanse based on the type
    ...
    def print_result q_id,pages
        ...
        result = "#{q_id}: "
        pages[0..4].each { |page_id| result << "#{page_id} "}
        p result
    end
    ...
    def calculate_page_value query,page
        ...
        for word in query.words
            if page.words.include? word
                page_value += query.word_weight[word] * page.word_weight[word]
            end
        end
        page_value
    end
